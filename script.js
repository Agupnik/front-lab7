"use strict";
document.querySelector('#show').addEventListener("click", () => showFromFile("profile_list.json"));
document.querySelector('#download').addEventListener("click", () => showFromServer(5));
...
function buildCards(profile) {
    ...
    c_latitude.innerText = "Coordinates latitude: " + profile.results[0].location.coordinates.latitude;
    let c_longitude = document.createElement('p');
    c_longitude.innerText = "Coordinates longitude: " + profile.results[0].location.coordinates.longitude;
    ...
    street_number.innerText = "Street number: " + profile.results[0].location.street.number;
    let street_name = document.createElement('p');
    street_name.innerText = "Street name: " + profile.results[0].location.street.name;

    let timezone_description = document.createElement('p');
    timezone_description.innerText = "Timezone description: " + profile.results[0].location.timezone.description;
    let timezone_offset = document.createElement('p');
    timezone_offset.innerText = "Timezone offset: " + profile.results[0].location.timezone.offset;
...
function showFromServer(count) {
    Promise.all(apiRequestLoop(count)).then(value => {
        status_bar.innerText = "Successfully downloaded elements from server! Count: " + count;
